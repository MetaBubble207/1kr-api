...
class AtkXQZ1703563311003 implements MigrationInterface {
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(
            `CREATE TABLE \`social_circle_sections\` (\`id\` varchar(36) NOT NULL, \`createdAt\` datetime(6) NOT NULL COMMENT '创建时间' DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL COMMENT '更新时间' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL COMMENT '删除时间', \`title\` varchar(255) NOT NULL COMMENT '标题' DEFAULT '', \`content\` text NOT NULL COMMENT '内容', \`free\` tinyint NOT NULL COMMENT '是否可试看' DEFAULT 0, \`duration\` int UNSIGNED NOT NULL COMMENT '时长' DEFAULT '0', \`online\` tinyint NOT NULL COMMENT '是否上线' DEFAULT 0, \`sequence\` int NOT NULL COMMENT '索引' DEFAULT '1', \`chapterId\` varchar(36) NOT NULL, INDEX \`idx_chapter_createdAt\` (\`chapterId\`, \`createdAt\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
        ...
        await queryRunner.query(
            `CREATE TABLE \`social_circle_posts\` (\`id\` varchar(36) NOT NULL, \`createdAt\` datetime(6) NOT NULL COMMENT '创建时间' DEFAULT CURRENT_TIMESTAMP(6), \`updatedAt\` datetime(6) NOT NULL COMMENT '更新时间' DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), \`deletedAt\` datetime(6) NULL COMMENT '删除时间', \`business\` enum ('feed', 'forum', 'course') NOT NULL COMMENT '业务', \`sectionId\` varchar(255) NOT NULL DEFAULT '', \`title\` varchar(255) NOT NULL COMMENT '标题' DEFAULT '', \`content\` text NOT NULL COMMENT '内容', \`commentCount\` int NOT NULL COMMENT '评论数' DEFAULT '0', \`likeCount\` int NOT NULL COMMENT '点赞数' DEFAULT '0', \`repostCount\` int NOT NULL COMMENT '转发数' DEFAULT '0', \`collectCount\` int NOT NULL COMMENT '收藏数' DEFAULT '0', \`disableComment\` tinyint NOT NULL COMMENT '是否禁止新评论' DEFAULT 0, \`userId\` varchar(36) NOT NULL, \`circleId\` varchar(36) NOT NULL, INDEX \`idx_circle_likeCount\` (\`circleId\`, \`likeCount\`), INDEX \`idx_circle_commentCount\` (\`circleId\`, \`commentCount\`), INDEX \`idx_circle_section_createdAt\` (\`circleId\`, \`sectionId\`, \`createdAt\`), INDEX \`idx_circle_createdAt\` (\`circleId\`, \`createdAt\`), INDEX \`idx_user\` (\`userId\`), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`,
        );
        await queryRunner.query(
